...
//				List<Integer> distinctContextInfo = contextInfoList.stream().distinct().collect(Collectors.toList());
	...
	public void fixWithMatchedFixTemplates(SuspCodeNode scn, List<Integer> distinctContextInfo) {
		...
		FixTemplate ft = null;
		String dataType_fix_pattern = "/Pattern_info/" + buggyProject;
		if (!Checker.isMethodDeclaration(scn.suspCodeAstNode.getType())) {
			...
			for (Integer contextInfo : distinctContextInfo) {
				...
				if (Checker.isCastExpression(contextInfo)) {
					...
					if (isTestFixPatterns) dataType = dataType_fix_pattern + "/ClassCastChecker"; //fp1
					...
					if (!typeChanged) {
						...
						if (isTestFixPatterns) dataType = dataType_fix_pattern + "/DataTypeReplacer";
					}
...
//						if (!isTestFixPatterns && this.minErrorTest == 0) return;
						...
						if (isTestFixPatterns) dataType = dataType_fix_pattern +"/MethodInvocationMutator";
					}
					...
					if (Checker.isInfixExpression(scn.suspCodeAstNode.getChild(0).getType()) && !operator) {
						...
						if (isTestFixPatterns) dataType = dataType_fix_pattern +"/OperatorMutator";
						generateAndValidatePatches(ft, scn);
						if (!isTestFixPatterns && this.minErrorTest == 0) return;
						if (this.fixedStatus == 2) {
							fixedStatus = 0;
					...
					if (isTestFixPatterns) dataType = dataType_fix_pattern +"/ConditionalExpressionMutator";
					...
					if (isTestFixPatterns) dataType = dataType_fix_pattern +"/ICASTIdivCastToDouble";
					generateAndValidatePatches(ft, scn);
					if (!isTestFixPatterns && this.minErrorTest == 0) return;
					if (this.fixedStatus == 2) {
						fixedStatus = 0;
					...
					if (isTestFixPatterns) dataType = dataType_fix_pattern +"/LiteralExpressionMutator";
					...
					if (Checker.isMethodInvocation(contextInfo)) {
						...
						if (isTestFixPatterns) dataType = dataType_fix_pattern +"/NPEqualsShouldHandleNullArgument";
						generateAndValidatePatches(ft, scn);
						if (!isTestFixPatterns && this.minErrorTest == 0) return;
						if (this.fixedStatus == 2) {
							fixedStatus = 0;
						...
						if (isTestFixPatterns) dataType = dataType_fix_pattern +"/RangeChecker";
					}
					...
					if ("boolean".equalsIgnoreCase(returnType)) {
						...
						if (isTestFixPatterns) dataType = dataType_fix_pattern +"/ReturnStatementMutator";
					}
				} else if (Checker.isSimpleName(contextInfo) || Checker.isQualifiedName(contextInfo)) {
					ft = new VariableReplacer();
					if (isTestFixPatterns) dataType = dataType_fix_pattern +"/VariableReplacer";
					...
					if (!nullChecked) {
						...
						if (isTestFixPatterns) dataType = dataType_fix_pattern +"/NullPointerChecker";
					}
				} 
				if (ft != null) {
					generateAndValidatePatches(ft, scn);
			...
			if (isTestFixPatterns) dataType = dataType_fix_pattern +"/StatementMover";
			generateAndValidatePatches(ft, scn);
			if (!isTestFixPatterns && this.minErrorTest == 0) return;
			if (this.fixedStatus == 2) {
				fixedStatus = 0;
			...
			if (isTestFixPatterns) dataType = dataType_fix_pattern +"/StatementRemover";
			generateAndValidatePatches(ft, scn);
			if (!isTestFixPatterns && this.minErrorTest == 0) return;
			if (this.fixedStatus == 2) {
				fixedStatus = 0;
			...
			if (isTestFixPatterns) dataType = dataType_fix_pattern +"/StatementInserter";
			generateAndValidatePatches(ft, scn);
			if (!isTestFixPatterns && this.minErrorTest == 0) return;
			if (this.fixedStatus == 2) {
				fixedStatus = 0;
...
//		System.out.println(dataType + " ====== " + patchCandidates.size());
	...
	public List<Integer> readAllNodeTypes(ITree suspCodeAstNode) {
		System.out.println(suspCodeAstNode);
